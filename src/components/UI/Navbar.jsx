...
const Navbar = () => {
  ...
  return (
    <div className="w-full bg-white top-0 left-0 z-50 shadow-md h-[100px] xl:h-[140px]">
      ...
      {/* Mobile Menu */}
      <div
        className={`xl:hidden transition-all duration-300 ease-in-out relative z-50 ${
          mobileMenuOpen
            ? "max-h-[600px] bg-primary  opacity-100"
            : "max-h-0 opacity-0"
        }`}
      ...
      >
        <div className="flex flex-col p-4 space-y-4 text-sm text-gray-700 font-oswald-medium font-light">
          ...
          <div className="flex flex-col flex-wrap gap-2 text-[15px] font-[500] font-helvetica text-[#707070] border-b-[3px] pb-4 border-b-white">
            {topLinks.map((link, index) => {
              const isExternal = link.to.startsWith("http");
